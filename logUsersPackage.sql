...
function updatePassword(userID int,oldPass varchar, newPass varchar) return int;
...
passDB varchar2(256);
begin
 select pass into passDB from Users where username like usernam;
  if pass = passDB then
  return 1;
  end if;
  return 0;
...
function updatePassword(userID int,oldPass varchar, newPass varchar) return int as
passDB varchar2(256);
begin
 select pass into passDB from Users where ID like userID;
 if oldPass = passDB then
  update Users set pass = newPass where ID = userID;
  return 1;
  end if;
  return 0;
  exception
